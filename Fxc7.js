...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Alip\n'
            + 'ORG:Owner PEKORA;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6282125986924:+62 821-2598-6924\n'
            + 'END:VCARD'

prefix = "!"
name = "~ PEKORA BOT"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN PEKORABOT😁"
rmenu = "HAI TEMEN PEKORABOT👋 JANGAN LUPA DONASI YAA:)"
botinfo = "UNTUK INVITE BOT SILAHKAN DONASI DULU YAA:)"
limitt = 100
memberLimit = 2
ban = []
premium = ["6282125986924@s.whatsapp.net","6282125986924@s.whatsapp.net","6283820073017@s.whatsapp.net","6285240750713@s.whatsapp.net","628826421519@s.whatsapp.net"]
...
async function starts() {
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]} PEKORA MISS YOU:D`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6282125986924@s.whatsapp.net"] // owner number ubah aja
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		case 'daftar':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Temen PEKORABOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    frhan.sendMessage('6282125986924@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat PEKORABOT\`\`\``)
					break
                                case 'bcgc':
					frhan.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *PEKORABOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[PEKORABOT]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
